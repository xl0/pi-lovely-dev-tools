...
Rendering uses 10 lines per cell with half-cell braille resolution (left column for lines 1-5, right column for 6-10), no cell cap, count glyphs (`ˍ` unread, then bottom-up rows up to `⣿` for 4+ reads per half), `borderAccent` coloring for startup context and advertised skills, `accent` coloring for injected skill bodies, and recency coloring for read-tool evidence. Wide terminals use a fixed 50-column middle-truncated filename column with aligned bars; filenames carry OSC8 `file://` links to full absolute paths on the visible path text only, and non-empty bar cells carry OSC8 links to the first line represented by that cell. Narrow terminals put bars under filenames.
