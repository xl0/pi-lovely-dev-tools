...
`/llm-stats` waits for idle, scans `ctx.sessionManager.getBranch()` for finalized assistant message entries with `usage`, and emits one displayed custom message hidden from LLM context. Each row represents one assistant/LLM call, not one tool call. Prompt-side tokens render as `fresh + cacheR = input`, where `fresh` is `usage.input` and `cacheR` is `usage.cacheRead`. Rows also show session entry id, `provider/model`, inferred start (`user`, `tools`, or `other`), output tokens, stop reason, and comma-separated tool calls or `-`.
